...
## 2026-05-17 - [Cache WindowsIdentity System Calls]
**Learning:** `WindowsIdentity.GetCurrent().Name` makes expensive Windows interop calls to look up the current user. Calling it repeatedly on every request adds significant CPU overhead and latency.
**Action:** Always cache OS-level identity lookups like `WindowsIdentity.GetCurrent().Name` in a static or singleton scope, as the user running the process generally doesn't change during the application's lifetime.
