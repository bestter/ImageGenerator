...
## 2026-08-10 - Protect Against Path Traversal by Directory Constraint
**Vulnerability:** Methods saving internal files constructed paths by simply sanitizing leaf strings and calling `Path.Combine()`. A malicious or forged filename could include `..` or traverse upwards when interpreted by `Path.GetFullPath()`.
**Learning:** `Path.GetInvalidFileNameChars()` is insufficient if the string is still used in contexts that resolve traversal operators, and `Path.Combine` doesn't restrict traversal.
**Prevention:** In internal file paths (like API keys or history) that have a fixed, designated directory, calculate the ultimate fully qualified path using `Path.GetFullPath()`. Then normalize the designated root directory, ensure it ends with a separator, and strictly enforce `StartsWith(..., StringComparison.OrdinalIgnoreCase)` on the fully qualified path before permitting the operation.
