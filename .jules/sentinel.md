...
## 2026-08-24 - Prevent Path Traversal By Appending Directory Separator
**Vulnerability:** When verifying if a resolved path resides within an intended directory using `StartsWith`, the application failed to ensure the target directory string ended with a directory separator (`Path.DirectorySeparatorChar`). If the expected directory is `C:\Temp\Folder` and an attacker crafts a path to `C:\Temp\Folder2\file.txt`, `StartsWith` returns true because "C:\Temp\Folder2" starts with "C:\Temp\Folder", resulting in a path traversal bypass that allows writing to sibling directories.
**Learning:** `StartsWith` does path boundary checks based on string characters, not directory hierarchies. A string that shares a prefix but represents a different path component will bypass this check if the separator isn't enforced.
**Prevention:** Always append `Path.DirectorySeparatorChar` to the expected directory string before using `StartsWith` for path traversal mitigation.
